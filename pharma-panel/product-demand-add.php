<?php
	...
	$pageName = "Customer Order Request";
	$parentPageURL = 'product-demand-master.php';
	$pageURL = 'product-demand-add.php';
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueDonationRequestById($id);
	}
	if(isset($_POST['id']) && !empty($_POST['id'])) {
        //update to database
        $result = $admin->updateProductDemandOrder($_POST,$_FILES);
			...
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i>Customer Order Details</h6>
					</div>
						...
						<div class="form-group">
                            <div class="row">
								...
								<div class="col-sm-3">
                                    ...
                                    <input readonly type="text" name="product_name" id="product_name" class="form-control" value="<?php if(isset($_GET['edit'])){ echo $data['product_name']; } ?>" required/>
                                </div>
								<div class="col-sm-3">
                                    <label>Brand Name</label>                                
                                    <input readonly type="text" name="band_name" id="band_name" class="form-control" value="<?php if(isset($_GET['edit'])){ echo $data['band_name']; } ?>" required/>
                                </div>
								<div class="col-sm-3">
                                    <label>Quantity</label>                                
                                    <input readonly type="text" name="qty" id="qty" class="form-control" value="<?php if(isset($_GET['edit'])){ echo $data['qty']; } ?>" required/>
                                </div>
                            </div>
                            </div>
                            <br>
                            <div class="row">
								...
								<div class="col-sm-3">
                                    <label>Name</label>                                
                                    <input readonly type="text" name="user_name" id="user_name" class="form-control" value="<?php if(isset($_GET['edit'])){ echo $data['user_name']; } ?>" required/>
                                </div>
								<div class="col-sm-3">
                                    <label>Contact / Email</label>                                
                                    <input readonly type="text" name="contact" id="contact" class="form-control" value="<?php if(isset($_GET['edit'])){ echo $data['contact']; } ?>" required/>
                                </div>
								<div class="col-sm-3">
                                    <label>Status</label>                                
                                    <select name="status" id="status" class="form-control" required/>
										<option value="0">Requested</option>
										<option value="1" <?php if(isset($_GET['edit']) && $data['status']==1){ echo "selected"; } ?>>Accepted</option>
										<option value="2" <?php if(isset($_GET['edit']) && $data['status']==2){ echo "selected"; } ?>>Rejected</option>
									</select><br>
                                </div>
                                <div class="col-sm-6">
                                    <label>Full Address</label>
                                    <textarea readonly col="5" rows="4"  class="form-control" name="address" id="address" required><?php if(isset($_GET['edit'])){ echo $data['address']; }  ?></textarea>
                                </div>
                            </div>
						</div>	
