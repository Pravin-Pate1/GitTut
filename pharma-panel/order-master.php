<?php
	...
	if(isset($_GET['delid']) && !empty($_GET['delid'])){
		...
		$sql = "SELECT * FROM ".PREFIX."order WHERE id='".$delid."'";
		$result = $admin->query($sql);
		if($admin->num_rows($result)>0){
			$Upsql = "DELETE FROM ".PREFIX."order WHERE `id`='".$delid."'";
			$admin->query($Upsql);
...
?>
							<tr>
								...
								<td>
									<?php 
										if($row['order_accept']==1){ ?>
											<span class="badge badge-success" style="background-color: #28a745">Accepted</span>
									<?php 	
										}elseif($row['order_accept']==2){ ?>
											<span class="badge badge-danger" style="background-color: #dc3545">Rejected</span>
									<?php 
										}else{ ?>
											<span class="badge badge-primary" style="background-color: #17a2b8;">Under Review</span>
									<?php 
										} ?>	
								</td>
								<td>
									<?php 
										if($row['order_accept']=="Completed"){ ?>
											<span class="badge badge-success" style="background-color: #28a745">Completed</span>
									<?php 	
										}elseif($row['order_accept']=="Shipped"){ ?>
											<span class="badge badge-danger" style="background-color: #dc3545">Shipped</span>
									<?php 
										}else{ ?>
											<span class="badge badge-primary" style="background-color: #17a2b8;">In Process</span>
									<?php 
										} ?>
								</td>
